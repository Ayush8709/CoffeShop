const myProduct = [
        {
        "title": "Black Coffee",
        "description": "Svart coffee is the same as the one you can drink with a small amount of coffee and drink it in vatten, serverat varmt. Oh, if you really like it, you can have a nice coffee with the right name: café noir.",
        "ingredients": [
        "Coffee"
        ],
        "image": "https://images.unsplash.com/photo-1494314671902-399b18174975?auto=format&fit=crop&q=80&w=1887&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
        "id": 1,
        "price":120
        },
        {
        "title": "Latte",
        "description": "This is the most popular kaffe brewery and is served with a latte and a small espresso and also with a glass of water. The kan beställas utan smak eller med smak av allt från vanilj till pumpa kryddor.",
        "ingredients": [
        "Espresso",
        "Ångad mjölk"
        ],
        "image": "https://images.unsplash.com/photo-1561882468-9110e03e0f78?auto=format&fit=crop&q=60&w=800&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8MTl8fGxhdHRlfGVufDB8fDB8fHww",
        "id": 2,
        "price":180,
        },
        {
        "title": "Caramel Latte",
        "description": "If you have a latte with a special smak, you can use caramel latte as an alternative for att ge dig en upplevelse av den Naturliga sötman och krämigheten hos ångad mjölk och caramel.",
        "ingredients": [
        "Espresso",
        "Ångad mjölk",
        "Karamellsirap"
        ],
        "image": "https://images.unsplash.com/photo-1599398054066-846f28917f38?auto=format&fit=crop&q=80&w=1887&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
        "id": 3,
        "price":260
        },
        {
        "title": "Cappuccino",
        "description": "Cappuccino is a latte that can be served with a small amount of milk, often with a stream of cocoa powder or something to top it off. Ibland kan you hitta variationer som vander grädde isället for mjölk or sådana som tillsätter smakamnen också.",
        "ingredients": [
        "Espresso",
        "Ångad mjölk",
        "Foam"
        ],
        "image": "https://images.unsplash.com/photo-1557006021-b85faa2bc5e2?auto=format&fit=crop&q=80&w=1887&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
        "id": 4,
        "price": 300
        },
        {
        "title": "Americano",
        "description": "The coffee is served with American coffee and espresso and is served very quickly",
        "ingredients": [
        "Espresso",
        "Hett vatten"
        ],
        "image": "https://images.unsplash.com/photo-1532004491497-ba35c367d634?auto=format&fit=crop&q=80&w=1887&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
        "id": 5,
        "price":600,
        },
        {
        "title": "Espresso",
        "description": "The espresso coffee can be served as a base for the flesta coffee maker, latte or macchiato.",
        "ingredients": [
        "Espresso"
        ],
        "image": "https://images.unsplash.com/photo-1579992357154-faf4bde95b3d?auto=format&fit=crop&q=80&w=1887&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
        "id": 6,
        "price":550
        },
        {
        "title": "Macchiato",
        "description": "Macchiaton är en annan espresso-baserad dryck som har en liten mängd skum på toppen. Det är det glada mellanrummet mellan en cappuccino och en doppio.",
        "ingredients": [
        "Espresso",
        "Foam"
        ],
        "image": "https://images.unsplash.com/photo-1557772611-722dabe20327?auto=format&fit=crop&q=80&w=1887&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
        "id": 7,
        "price":330,
        },
        {
        "title": "Mocha",
        "description": "For all the chokladälskare, there is no need for the förälskade in mocha. Mocha is a choklad-espresso drink with the same mjölk and skum.",
        "ingredients": [
        "Espresso",
        "Ångad mjölk",
        "Choklad"
        ],
        "image": "https://images.unsplash.com/photo-1607260550778-aa9d29444ce1?auto=format&fit=crop&q=80&w=1887&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
        "id": 8,
        "price":400
        },
        {
        "title": "Hot Chocolate",
        "description": "Under kalla winterdagar for en kopp varm choklad dig att känna dig bekväm och lycklig. This means that the energizing caffeine contained in it is also strong.",
        "ingredients": [
        "Choklad",
        "Mjölk"
        ],
        "image": "https://images.unsplash.com/photo-1542990253-0d0f5be5f0ed?auto=format&fit=crop&q=60&w=800&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8NDh8fGhvdCUyMGNob2NvbGF0ZXxlbnwwfHwwfHx8MA%3D%3D",
        "id": 9,
        "price":350,
        },
        {
        "title": "Chai Latte",
        "description": "Om you letar efter en smakfull varm dryck mitt i winter, preferably chai latte. Combinations av kardemumma och kanel ger en underbar smak.",
        "ingredients": [
        "Te",
        "Mjölk",
        "Ingefära",
        "Kardemumma",
        "Kanel"
        ],
        "image": "https://images.unsplash.com/photo-1578899952107-9c390f1af1b7?w=900&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8MTJ8fGNoYWklMjBsYXR0ZXxlbnwwfHwwfHx8MA%3D%3D",
        "id": 10,
        "price":480
        },
        {
        "title": "Matcha Latte",
        "description": "Matcha latte is in green, hälsosam kaffedryck with finkrossad matcha-te and mjölk, erbjuder mild sotma, in unique smak and in a mild caffeine kick.",
        "ingredients": [
        "Matcha-pulver",
        "Mjölk",
        "Socker*"
        ],
        "image": "https://images.unsplash.com/photo-1536256263959-770b48d82b0a?w=900&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8M3x8bWF0Y2hhJTIwbGF0dGV8ZW58MHx8MHx8fDA%3D",
        "id": 11,
        "price":750
        },
        {
        "title": "Seasonal Brew",
        "description": "Sasongs coffee with olika smaktoner som caramel, fruit and chocolate",
        "ingredients": [
        "Kaffe"
        ],
        "image": "https://images.unsplash.com/photo-1611162458324-aae1eb4129a4?w=900&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8MTg1fHxibGFjayUyMGNvZmZlZXxlbnwwfHwwfHx8MA%3D%3D",
        "id": 12,
        "price":700,
        },
        {
        "title": "Svart Te",
        "description": "Svart te föddes i Kina. Det är tillverkat av blad från en växt som kallas Camellia och kan smaksättas olika med frukter till exempel. En trevlig, varm, smakfull och aromatisk dryck som passar till vardagen.",
        "ingredients": [
        "Te"
        ],
        "image": "https://images.unsplash.com/photo-1576092768241-dec231879fc3?auto=format&fit=crop&q=60&w=800&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8MjB8fHRlYXxlbnwwfHwwfHx8MA%3D%3D",
        "id": 13,
        "price":705
        },
        {
        "title": "Islatte",
        "description": "Iced latte is served with kyld kaffedryck as the görs genome att blanda espresso and kyld mjölk. The servers with isbitar and you can also enjoy the café latte iced or latte on the rocks.",
        "ingredients": [
        "Espresso",
        "Mjölk",
        "Is",
        "Sirap"
        ],
        "image": "https://images.unsplash.com/photo-1517701550927-30cf4ba1dba5?auto=format&fit=crop&q=60&w=800&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8NHx8aWNlZCUyMGxhdHRlfGVufDB8fDB8fHww",
        "id": 14,
        "price":220,
        },
        {
        "title": "Islatte Mocha",
        "description": "Iced latte Mocha is a combination of latte and mocha, som is a unique combination of chocolate and coffee. The ger kalla dryckälskare and läcker upplevelse av choklad och coffee.",
        "ingredients": [
        "Espresso",
        "Is",
        "Mjölk",
        "Choklad "
        ],
        "image": "https://images.unsplash.com/photo-1642647391072-6a2416f048e5?auto=format&fit=crop&q=60&w=800&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8Mzh8fGljZWQlMjBtb2NoYSUyMGxhdHRlfGVufDB8fDB8fHww",
        "id": 15,
        "price":100,
        },
        {
        "title": "Frapino Caramel",
        "description": "The air on the blandad eller bättre says skakad coffee with a view of the top. Ett måste for varma sommardagar.",
        "ingredients": [
        "coffee",
        "Is",
        "Mjölk",
        "Karamellsirap",
        "Vispgrädde*",
        "Karamellsås"
        ],
        "image": "https://images.unsplash.com/photo-1662047102608-a6f2e492411f?auto=format&fit=crop&q=60&w=800&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8NHx8ZnJhcGlubyUyMGNhcmFtZWx8ZW58MHx8MHx8fDA%3D",
        "id": 16,
        "price":190
        },
        {
        "title": "Frapino Mocka",
        "description": "You can also use it to get it dry before you get it. Tänk dig smaken a en shake with chocolate and vispad gratde på toppen.",
        "ingredients": [
        "Coffee",
        "Is",
        "Mjölk",
        "Cocoa",
        "Vispgrädde*"
        ],
        "image": "https://images.unsplash.com/photo-1530373239216-42518e6b4063?auto=format&fit=crop&q=60&w=800&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8NHx8ZnJhcGlubyUyMG1vY2hhfGVufDB8fDB8fHww",
        "id": 17,
        "price":680,
        },
        {
        "title": "Apelsinjuice",
        "description": "You can also read about the pressure of apelsinjuice. You'll have to see the själv." [
        "Färska Apelsiner",
        "Is"
        ],
        "image": "https://images.unsplash.com/photo-1600271886742-f049cd451bba?auto=format&fit=crop&q=60&w=800&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8NzF8fG9yYW5nZSUyMGp1aWNlfGVufDB8fDB8fHww",
        "id": 18,
        "price":1000
        },
        {
        "title": "Frozen Lemonade",
        "description": "Frozen lemonade is a delicious sommardryck with a combination of pressed lemon juice, it is also very tasty and a delicious sensation.",
        "ingredients": [
        "Citronsaft",
        "Is",
        "Socker*"
        ],
        "image": "https://images.unsplash.com/photo-1523371054106-bbf80586c38c?w=900&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8MTZ8fGxlbW9uYWRlJTIwd2l0aCUyMGljZXxlbnwwfHwwfHx8MA%3D%3D",
        "id": 19,
        "price":880
        },
        {
        "title": "Lemonad",
        "description": "Var can be in Paris for the first time and the sedan mycket is popular in Europe. Denna söta, färglösa, kolsyrade dryck görs genome att blanda lemon juice and kolsyrat vatten.",
        "ingredients": [
        "Citronsaft",
        "Kolsyrat vatten",
        "Honung"
        ],
        "image": "https://images.unsplash.com/photo-1621263764928-df1444c5e859?auto=format&fit=crop&q=60&w=800&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8Nnx8bGVtb25hZGV8ZW58MHx8MHx8fDA%3D",
        "id": 20,
        "price":710
        }
        ]

export default myProduct;